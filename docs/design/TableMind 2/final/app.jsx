...
function Intro() {
  return (
    <DCSection
      title="TableMind · Бренд-система — D / Карточка меню (финал)"
      ...
      <DCArtboard label="Философия" width={720} height={560}>
        <div style={{ background: T.paper, width: '100%', height: '100%', padding: 48, display: 'flex', flexDirection: 'column', gap: 20 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', ...TX.masthead, paddingBottom: 10, borderBottom: `1.5px solid ${T.ink}` }}>
            <span>TABLEMIND · №003</span>
            <span>ВЫПУСК · 20·IV·2026</span>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, ...TX.small }}>
            <div><b style={{ color: T.ink }}>Основной</b> — Мох {T.moss}</div>
            <div><b style={{ color: T.ink }}>Акцент</b> — Охра {T.ochre}</div>
            <div><b style={{ color: T.ink }}>Бумага</b> — Овсяная {T.paper}</div>
            <div><b style={{ color: T.ink }}>Чернила</b> — {T.ink}</div>
            <div><b style={{ color: T.ink }}>Заголовки</b> — Playfair курсив</div>
            <div><b style={{ color: T.ink }}>Текст</b> — DM Sans</div>
          </div>
          <div style={{ flex: 1 }} />
...
function Final() {
  return (
    <DesignCanvas>
      <Intro />
      <DCSection title="Айдентика" subtitle="Логотип, палитра, типографика, компоненты" gap={56}>
        <LogosArtboard />
        <ColorArtboard />
        <TypeArtboard />
        <ComponentsArtboard />
      </DCSection>
      <DCSection title="Применение" subtitle="Маркетинговый сайт (B2B) и гостевой PWA" gap={56}>
        <SiteMock />
        <PwaMenuArtboard />
        <PwaChatArtboard />
        <PwaWaiterArtboard />
