export function formatCatalogForAI(items) {
	return items.map(item => {
		const allergens = item.allergens.length > 0 ? item.allergens.join(", ") : "не указаны";
		const tags = item.tags.length > 0 ? item.tags.join(", ") : "";
		return `[${item.id}] ${item.name} | ${item.category} | ${item.price}₽ | ${item.weight} | ${item.description} | Аллергены: ${allergens}${tags ? " | Теги: " + tags : ""}`;
	}).join("\n");
}
